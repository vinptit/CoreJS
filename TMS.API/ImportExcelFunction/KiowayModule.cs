...
namespace TMS.API.ImportExcelFunction
{
    ...
    public class KiowayModule
    {
        private static FastKiowayContext _fast;
        ...
        public KiowayModule(IHttpContextAccessor httpContextAccessor) 
        {
            ...
            _fast = serviceProvider.GetRequiredService<FastKiowayContext>();
            db = serviceProvider.GetRequiredService<TMSContext>();
            FastTenant = "FastKioway";
        ...
        public async Task<string> AddOrUpdateList(List<ImportChargeVM> fee)
        {
            var errorMessage = new StringBuilder();
            var fileId = db.ImportExcel.Where(x => path.Contains(x.Attachment)).OrderByDescending(x => x.Id).Select(x => x.Id).FirstOrDefault();
            try
            {
                foreach (var item in fee)
                {
                    ...
                    var newRes = new ImportExcelResult()
                    {
                        ...
                        TYPE = item.TYPE,
                        Status = item.Status,
                        StatusFeeText = item.StatusFeeText,
                        FileImportId = fileId,
                    };
